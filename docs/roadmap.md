...
- **实施计划**：见 [superpowers/plans/2026-08-22-phase1-file-operations.md](superpowers/plans/2026-08-22-phase1-file-operations.md)
...
> 📌 **当前进度**：第一轮（浏览历史 + 评分 + 增量扫描）于 2026-08-16 完成。第二轮（文件操作）实施计划已就绪，见 [#3 图片文件操作](#3-图片文件操作-)。
...
| 隐患 | 影响 | 修复方向 |
|------|------|----------|
| 收藏/历史以 `image_path` 字符串为主键 | 文件重命名/移动后引用失效 | 实现路径级联更新机制（见 #3） |
| `imageStore` 已成巨石 store | 承载库/图片/收藏/文件夹树/视图/排序 | 后续拆分为 `libraryStore`/`viewStore` |
| 无配置持久化层 | 主题/缓存/快捷键无法保存 | 引入 `electron-store`（见 #21） |
| `media-registry` 令牌为内存态 | 重启后失效，无法做分享/书签 | 远期需求时再持久化 |
| 文件夹级增量扫描被暂缓 | 超大库全量扫描成本高（10万张约 7.6 分钟） | 需 watch/归档位机制，暂不实现 |

### 工程化欠账

- `scripts/` 目录为空，测试数据生成脚本未创建
- 归档测试方案中 `TC-THUMB-004/005`、`TC-GRID-001~004`、`TC-PERF-001~003` 未执行
- 单元测试（50 用例）可持续扩充
...
**第二轮 (1 周)**：图片文件操作 + 批量重命名 + 回收站 + 设置壁纸（合并实现，见 [#3 实施计划](superpowers/plans/2026-08-22-phase1-file-operations.md)）
