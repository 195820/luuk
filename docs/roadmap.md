...
> 📌 **当前进度**：第一轮（浏览历史 + 评分 + 增量扫描）于 2026-08-16 完成。第二轮（文件操作）于 2026-08-30 完成。第三轮为技术债清理（Phase 1.5），见 [phase1.5-tech-debt.md](superpowers/plans/2026-08-30-phase1.5-tech-debt.md)。
...
- [ ] 归档测试方案中的待测试用例（见 [archive/测试方案.md](archive/测试方案.md)）：`TC-THUMB-004/005`、`TC-GRID-001~004`、`TC-PERF-001~003`（Phase 1.5 Task 4 执行中）
- [x] `scripts/` 目录：`generate-test-data.cjs` 已创建
- [ ] 补充自动化单元测试（当前 60+ 用例，Phase 1.5 Task 3 扩充多选/边界用例）
...
| `imageStore` 已成巨石 store | 承载库/图片/收藏/文件夹树/视图/排序 | 🟡 拆分中 — `selectionStore` + `viewStore`（Phase 1.5 Task 6-7） |
