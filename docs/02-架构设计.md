...
window.electronAPI = {
  ...
  // 媒体加载
  loadFullImage,   // 图片 → data URL
  getMediaUrl,     // 视频/音频 → data URL (base64)
...
> **⚠️ 2026-06-22 注意**：当前 data URL 方案对大文件存在 OOM 风险（500MB 视频 → 667MB 内存）。计划改用 `file://` 协议直连（Electron 原生支持）。详见 [11-多媒体模块重构方案.md](11-多媒体模块重构方案.md)。

### data: URL 方案（当前实现）

**注册位置**：`src/main/ipc/library-handlers.ts` → `getMediaUrl` IPC 处理器

**用途**：将视频/音频文件读取为 base64 编码的 data URL，浏览器原生支持播放。

**工作原理**：
1. 前端调用 `getMediaUrl(filePath)` IPC
2. 后端进行路径安全检查（确保在已注册库路径内）
3. 读取文件内容，根据扩展名设置正确的 MIME 类型
4. 返回 `data:{mimeType};base64,{base64Content}`

**安全检查**：
- 路径必须匹配已注册库的 `rootPath`
- 使用大小写不敏感比较（Windows 兼容）
- 文件不存在时抛出异常
- 库路径从数据库读取时使用 `.trim().replace(/\r/g, '')` 清理不可见字符
...
**优势**：零内存开销，Electron 渲染进程原生支持 `file://` 协议，`<video>` 和 `<audio>` 元素可直接加载本地文件。

**luuk-file:// 协议**：已在 `electron/main.ts` 中实现（支持 Range 请求），但 Chromium media pipeline 不支持自定义协议，因此视频/音频改用标准 `file://` 协议。`luuk-file://` 保留用于图片流式加载场景。
...
- 适应模式：适应窗口/实际大小
- 幻灯片播放：3/5/10/30 秒间隔
- **视频播放**：浏览器原生 `<video>` 标签（mp4/webm/mov）
- **音频播放**：浏览器原生 `<audio>` 标签
- 媒体类型通过 `mediaType` 字段自动识别，使用 `luuk-file://` 流式加载
