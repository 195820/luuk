...
window.electronAPI = {
  ...
  getMediaUrl,     // 视频/音频 → luuk-file:// 流式 URL
...
**注册位置**：`electron/main.ts` → `registerFileProtocol()`

**用途**：流式加载视频/音频/大图文件，替代 `file://` 协议。

**工作原理**：
1. 路径通过 `base64url` 编码：`luuk-file://${Buffer.from(path).toString('base64url')}`
2. Electron 自动处理 HTTP range 请求，支持大文件按需加载
3. 前端通过 `<video src="luuk-file://...">` 或 `<audio src="luuk-file://...">` 直接使用

**安全检查**：
- 请求到达时解析为绝对路径
- 与已注册库的 `rootPath` 逐一比对
- 路径不在任何库目录下 → 返回 403
- 文件不存在 → 返回 404
- 服务未初始化 → 返回 503
...
| 媒体类型 | 加载方式 | IPC 方法 |
|----------|---------|----------|
| 图片 | data URL（base64 内嵌） | `loadFullImage()` |
| 视频 | luuk-file:// 流式 URL | `getMediaUrl()` |
| 音频 | luuk-file:// 流式 URL | `getMediaUrl()` |
