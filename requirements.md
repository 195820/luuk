...
| 库切换 | 快速切换不同库 | P1 |
| 跨库搜索 | 统一入口搜索所有库 | P2 |
| 文件夹树浏览 | 按文件夹层级浏览图片 | P1 |
| 库路径冲突检测 | 防止在子文件夹中重复创建库 | P1 |
...
| 优化项 | 描述 | 预期提升 | 优先级 |
|--------|------|----------|--------|
| **并行扫描** | 将串行扫描改为批量并行处理（如每次 10-20 张图片） | 30-50% | P2 |
| **Hash 优化** | 文件去重 Hash 从 1MB 减少到 64KB | 10-15% | P3 |
| **元数据缓存** | 相同尺寸/格式的图片复用部分元数据 | 5-10% | P3 |
| **增量扫描优化** | 使用文件修改时间快速跳过未变动文件 | 80-90%（增量） | P1 |
| **数据库批处理** | 使用更大批量的事务插入（如每次 100 条） | 20-30% | P2 |
| **路径索引优化** | 为文件夹树浏览添加专用索引 | 50%（文件夹切换） | P2 |
